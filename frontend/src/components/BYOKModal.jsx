...
export function BYOKModal({ onClose }) {
  ...
  const [keyHints, setKeyHints] = useState(() =>
    JSON.parse(sessionStorage.getItem("angetic.key_hints") || "{}"),
  );

  const provider = useMemo(
    () =>
      visibleProviders.find((item) => item.id === active) ||
      visibleProviders[0],
    [active, visibleProviders],
  );
  const meta = PROVIDER_META[provider?.id] || {
    logo: provider?.id?.slice(0, 2)?.toUpperCase() || "AI",
    prefix: "",
    desc: "Model provider",
  };
  ...
  const mutation = useMutation({
    ...
    onSuccess: async (data) => {
      ...
      sessionStorage.setItem("angetic.key_hints", JSON.stringify(nextHints));
      ...
      <div className="modal" onClick={(event) => event.stopPropagation()}>
        <aside className="modal-side">
              ...
              return (
                ...
                >
                  <span className="provider-logo">
                    {itemMeta.logo || item.id.slice(0, 2).toUpperCase()}
                  </span>
          ...
          <div className="security-copy">
            <Icon.Lock /> Keys are sent once to the backend, validated,
            encrypted with `KEY_ENCRYPTION_SECRET`, and never logged.
          </div>
        </aside>
        ...
        <main className="modal-main">
          <div className="modal-head">
            ...
            <div>
              ...
              <div className="ph-desc">
                {provider?.unavailable_reason || meta.desc}
              </div>
            </div>
          ...
          <div className="modal-body">
            <div className="field">
              <div className="label">
                ...
                <span className="hint">
                  {keyHints[active]
                    ? `stored as ${keyHints[active]}`
                    : meta.prefix
                      ? `usually starts with ${meta.prefix}`
                      : "provider token"}
                </span>
              </div>
              ...
              <div className="help">
                The key is stored server-side as Fernet ciphertext. The browser
                keeps only the returned hint for this session.
              </div>
            </div>
            ...
            <div className="field">
              <div className="label">
                ...
                <span className="hint">
                  {provider?.available ? "ready" : "key required"}
                </span>
              </div>
            ...
            {mutation.isSuccess && (
              <div className="inline-success">Key validated and encrypted.</div>
            )}
          </div>
          ...
          <div className="modal-foot">
            <div className="ff-status">
              <span
                className={
                  provider?.available ? "status-dot good" : "status-dot"
                }
              />
              {provider?.available
                ? "Provider available"
            ...
            <div className="ff-actions">
              ...
              <button
                ...
                disabled={
                  !apiKey || !provider?.runtime_available || mutation.isPending
                }
              >
